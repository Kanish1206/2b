...
def process_reco(gst_df, pur_df, doc_threshold=85, tax_tolerance=10, gstin_mismatch_tolerance=20):
    ...
    # Define Required Columns (Comprehensive List)
    gst_required = [
    ...
    # ---------------- FUZZY MATCHING ----------------
    # (Processes matches within the same GSTIN)
    for gstin in merged["Supplier GSTIN"].dropna().unique():
        ...
        for left_idx in open_2b.index:
            ...
            if match:
                ...
                pur_columns = [col for col in merged.columns if col.endswith("_PUR") or col in ["Reference Document No.", "Vendor/Customer Name", "Vendor/Customer Code", "FI Document Number", "Taxable Amount"]]
    ...
    # ---------------- GSTIN MISMATCH CHECK (The logic you requested) ----------------
    # We look for rows with the same Doc Number but different GSTINs
    ...
    for left_idx in open_2b_final.index:
        ...
        # Look for the same document in books that is currently 'Open'
        ...
        for right_idx in possible.index:
            ...
            # Check if invoice values are close enough to be considered a mismatch of GSTIN only
